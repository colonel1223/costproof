...
from costproof.agent import guard, llm, tools
...
class AuditRecord:
    ...
    narrative_check: dict = field(default_factory=dict)
    narrative_rejected: str = ""
...
class CostReview:
    ...
    def _narrative_payload(self) -> str:
        """Exactly what the model is shown. Also the evidence the guard checks against."""
        payload = {
            "findings": [
                {
    ...
    def _narrate_and_check(self) -> None:
        """Ask the model to summarise, then refuse the summary if it is not grounded.

        The model receives only tool output. The guard then extracts every figure the
        model wrote and looks for it in that same tool output. A figure that is not
        there -- invented, rounded, combined -- rejects the whole narrative, the
        deterministic backend writes the section instead, and the audit record keeps
        the rejected text and the reason. The header can never credit a model for a
        narrative that failed, or for one that does not exist.
        """
        evidence = self._narrative_payload()
        prompt = ("Summarise this cost review for a finance stakeholder in under 200 "
                  "words.\n\n" + evidence)
        try:
            text = self.backend.generate(prompt, system=llm.SYSTEM_PROMPT)
        except Exception as exc:  # noqa: BLE001 -- a model failure must not fail the review
            text, err = "", f"{type(exc).__name__}: {str(exc)[:120]}"
        else:
            err = ""

        result = guard.check_narrative(text, evidence)
        if err:
            result.ok = False
            result.problems.insert(0, f"model call failed: {err}")

        self.audit.narrative_check = {
            "ok": result.ok, "problems": result.problems,
            "figures_checked": result.figures_checked,
            "figures_ungrounded": result.figures_ungrounded,
            "backend_attempted": self.backend.name,
        }
        if result.ok:
            self.audit.narrative = text
            self.audit.narrative_source = self.backend.name
        else:
            self.audit.narrative_rejected = text
            self.audit.narrative = llm.DeterministicBackend().generate(prompt)
            self.audit.narrative_source = (
                f"deterministic (narrative from {self.backend.name} rejected: "
                f"{result.summary()})"
            )
        if self.verbose:
            print(f"  narrative guard: {result.summary()}")
...
def render_markdown(audit: AuditRecord) -> str:
    ...
    out += ["", "---", "", "## Narrative summary",
            f"*Generated by `{audit.narrative_source}`*", ""]
    chk = audit.narrative_check or {}
    if chk:
        status = "passed" if chk.get("ok") else "FAILED"
        out += [f"*Grounding check {status}: every figure in the narrative must appear in "
                f"the tool output the model was shown "
                f"({chk.get('figures_checked', 0)} checked"
                + (f"; ungrounded: {', '.join(chk['figures_ungrounded'])}"
                   if chk.get("figures_ungrounded") else "") + ").*", ""]
    out += [audit.narrative or "_(no narrative)_", ""]
    if audit.narrative_rejected:
        out += ["<details><summary>Rejected model narrative (kept for audit)</summary>", "",
                audit.narrative_rejected, "", "</details>", ""]
    return "\n".join(out)
